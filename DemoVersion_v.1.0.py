...
"""
DEMO VERSION
Quickly prepare a demo version of the current font with minimal user input.
...
• .notdef control: inserts a standard replacement glyph if missing, or a custom one with the word DEMO. Both styles automatically adapt to the font’s cap height. • Demo Trap feature: replaces several sensitive glyphs to introduce friction in extended use. • Adds a License Custom Parameter noting demo restrictions. • Clears all OpenType features. • Simple and minimal interface.

Requirements:
• Glyphs 3 or later
• Vanilla (bundled with Glyphs)

Author:
• Denis Serebryakov (serebryakov.com)
"""
...
def make_trial_font(selected_prefix="Demo", apply_trial_trap=False, notdef_mode=0, open_in_glyphs=True):
    font = Glyphs.font
    if not font:
        Glyphs.showNotification("Demo Version", "Error! Open the source file before running the script.")
        return
...
# BUTTON EXPORT        
    ...
    def runScript(self, sender):
        ...
        Glyphs.showNotification("Demo Version", "Success! Source file is ready.")
        self.window.close()
